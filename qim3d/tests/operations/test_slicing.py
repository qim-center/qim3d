...
def test_random_slice_shape(volume):
    ...
    sl = qim3d.operations._slicing_operations.get_random_slice(
        volume, width=5, length=8, seed=0
    )
    assert sl.shape == (5, 8)
...
def test_random_slice_reproducible(volume):
    ...
    s1 = qim3d.operations._slicing_operations.get_random_slice(
        volume, width=6, length=4, seed=42
    )
    s2 = qim3d.operations._slicing_operations.get_random_slice(
        volume, width=6, length=4, seed=42
    )
...
def test_random_slice_differs_with_different_seed(volume):
    ...
    s1 = qim3d.operations._slicing_operations.get_random_slice(
        volume, width=6, length=4, seed=1
    )
    s2 = qim3d.operations._slicing_operations.get_random_slice(
        volume, width=6, length=4, seed=2
    )
...
def test_random_slice_within_bounds(volume):
    ...
    sl = qim3d.operations._slicing_operations.get_random_slice(
        volume, width=7, length=7, seed=7
    )
...
def test_get_random_slice_sanity(volume):
    ...
    sl = qim3d.operations._slicing_operations.get_random_slice(
        volume, width=7, length=4, seed=42
    )
    assert sl.shape == (7, 4)
    assert not np.isnan(sl).any()
