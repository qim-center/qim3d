...
class Interface(BaseInterface):
    def __init__(self, verbose: bool = False):
        ...
        """
        super().__init__(
            title='Volume generator', height=1024, width=900, verbose=verbose
        )
    ...
    def define_interface(self, gradio_interface, *args, **kwargs):
            ...
            with gr.Column():
                with gr.Group():
                    noise_type = gr.Dropdown(
                        ['Perlin', 'Simplex', 'PNoise', 'P', 'SNoise', 'S'],
                        label='Noise type',
                    )
                    ...
                    axis = gr.Slider(
                        0, 2, 0, step=1, visible=False, label='Axis of shape'
                    )
                ...
                with gr.Group():
                    ...
                    file_extensions = gr.Dropdown(
                        choices=['.tiff', '.nii.gz', '.h5', '.vol', '.dcm', '.zarr'],
                        value='.tiff',
                        label='File format',
                        interactive=True,
                    )
                ...
                # with gr.Row():
                with gr.Group():
                    generate_html = gr.Button(
                        'Save interactive plot', variant='primary'
                    )
        ...
        gr.on(
            triggers=[input.change for input in display_inputs],
            fn=self.plot_volume,
        ...
        generate_volume.click(
            fn=self.save_volume, inputs=file_extensions, outputs=volume_file
        )
